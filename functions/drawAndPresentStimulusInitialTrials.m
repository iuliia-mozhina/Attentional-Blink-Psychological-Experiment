function times = drawAndPresentStimulusInitialTrials(w, screens, const, times, rsvp, t1Pos, t1ColorName, intensity)
% FUNCTION times = drawAndPresentStimulusInitialTrials(w, screens, const, times, rsvp, t1Pos, t1ColorName, intensity)
%
...
% intensity: the recommended intensity of the presentation time words in RSVP stream
...
for i = 1:length(rsvp)
    % assign a color for the target adjective
	if i == t1Pos
	...
	t2s(cnt+1) = Screen('Flip', w, t2s(cnt) + intensity - deltat);
    t2s(cnt+2) = Screen('Flip', w, t2s(cnt+1) + times.ISI- deltat);
    cnt = cnt + 2;
end
Screen('FillRect', w, const.bgcolor, stimRect);
t3 = Screen('Flip', w, t2s(end) + intensity - deltat);
